...
test("Portal and workspaces share visual language, rebuild, and Changes", async ({ page }) => {
  ...
  try {
    ...
    const consolePanel = page.locator("#agent-console-panel");
    const consoleHandle = page.locator(".agent-console-resize-handle");
    const consoleWidth = await consolePanel.evaluate((element) => element.getBoundingClientRect().width);
    ...
    await page.mouse.up();
    await expect.poll(() => consolePanel.evaluate((element) => element.getBoundingClientRect().width)).toBeGreaterThan(consoleWidth + 60);
    await consoleHandle.focus();
    await consoleHandle.press("ArrowRight");
    await expect(consoleHandle).toHaveAttribute("aria-valuenow", /\d+/);
    ...
    await page.mouse.up();
    await expect.poll(() => sidebar.evaluate((element) => element.getBoundingClientRect().width)).toBeGreaterThan(sidebarWidth + 60);
    await sidebarHandle.focus();
    await sidebarHandle.press("ArrowLeft");
    await expect(sidebarHandle).toHaveAttribute("aria-valuenow", /\d+/);
