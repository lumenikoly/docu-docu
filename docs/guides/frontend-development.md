...
`appearance.ts`, `portal.ts` и `serve.ts` не имеют статической зависимости от
React, React DOM и Base UI. Editor и Changes работают как отдельные React
application roots. Roadmap загружается как eager
...
Исходники делятся на внутренние каталоги `design`, `ui`,
...
движение. CSS использует только канонические имена `--td-*`; темы задают те же
семантические переменные и не создают отдельную палитру для конкретной
возможности.
...
Итоговый язык остаётся инструментальным: тонкая граница разделяет области,
тень обозначает только поднятый слой, радиусы умеренны, а один акцент показывает
выбор и действие. Portal сохраняет широкую читательскую колонку и спокойную
иерархию содержимого; Editor и Changes используют более плотную сетку рабочей
области. Рабочая область задач, дорожная карта и обсуждения применяют те же
высоты элементов управления, обводку фокуса, формы, вкладки, диалоги и статусы.
Состояния загрузки, пустого результата и ошибки имеют текст или доступное
название и не передают смысл одним цветом.

Браузерные компоненты берут локальные контуры Lucide из
`web/src/design/icons.ts`, а сгенерированная Go-оболочка — из
`internal/app/icons.go`. Добавляйте новую иконку в соответствующий реестр и
создавайте её через `createIcon`, `Icon` или `renderIcon`. `Icon` и `renderIcon`
создают только декоративные значки с `aria-hidden="true"`; доступное название
принадлежит содержащей их кнопке или ссылке. `createIcon` также поддерживает
самостоятельную значимую иконку с `role="img"` и `aria-label`. Толщина линии
равна `2`; внешний пакет и загрузка из сети не нужны.
...
Диапазон, фильтры, список и detail shell хранятся в React; содержимое сравнений
приходит из Changes API. Не вычисляйте Git, rendered или semantic diff в
браузере. Для CodeMirror, Mermaid, polling и внешних событий всегда возвращайте
cleanup. Если polling обнаружил новую ревизию при открытом файле, сохраните
текущую деталь и предложите явное обновление.
