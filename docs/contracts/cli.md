...
`refresh`, `translate` и `clarify` нет: одноимённые `$toudocu` процессы
выполняет агент разработки, а не Go CLI.
...
- Любой настроенный locale root поддерживает полный CLI и `serve`; task,
  `scaffold` и Editor меняют только выбранный root.
...
ссылки на предков и непосредственных детей с их `workState` и сводкой потомков.
Text-формат `task context` показывает компактные ссылки с сохранёнными
статусами, наличие блокера и вычисленную сводку, не включая полные документы
потомков. `workState` ссылок доступен в JSON.
`TaskTreeReport` содержит `taskId` и рекурсивные узлы `id`, `status`,
`workState`, `descendants`, `title`, `children`. `descendants` содержит `total`,
объект `counts` для всех значений рабочего состояния, `started` и `complete`;
поле присутствует у узлов с потомками. Все эти отчёты сохраняют
`schemaVersion: 1`.
...
всегда содержит `workState` и добавляет `descendants` для кандидата с потомками;
...
одновременно действующие условия неготовности. Его основная метка берётся из
`workState`: полный Draft обозначается `READY CANDIDATE`, а неполный Ready —
`NEEDS ATTENTION`. Прогресс ветки выводится дополнительным условием и не меняет
`readyForWork`.
...
независимо от активной локали. Автоматизация должна опираться на стабильный
