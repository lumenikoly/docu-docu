...
в него компактный контекст задачи и команду получения полного актуального
контекста; браузер отвечает только за копирование и ручной fallback.
...
- «Спросить» создаёт разовый разговор в Agent Console, а «Обсудить» сохраняет
  сообщение и доставку в общей очереди Agent Feedback.
