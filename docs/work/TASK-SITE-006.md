...
- [ ] `AC-01` Все непосредственные дочерние задачи завершены, а старая и новая UI-основы не существуют параллельно.
- [ ] `AC-02` Обычная статическая страница не загружает React; Portal
  работает в корне и во вложенном URL.
- [ ] `AC-03` Выпущенный бинарный файл выполняет `check`, `build` и
  `serve` без Node.js.
- [ ] `AC-04` Документация описывает итоговую архитектуру; `make check`,
