...
- [ ] `AC-01` Discussions загружается и монтируется только после реального открытия.
- [ ] `AC-02` Компоненты сохраняют семантику серверной части, состояния,
  якоря, аренды, ответы и ошибки без второго источника истины.
- [ ] `AC-03` Компоненты и API пригодны для повторного использования внутри Changes.
- [ ] `AC-04` Клавиатура, фокус, закрытие, повторное открытие и мягкая
