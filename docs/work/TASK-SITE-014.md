...
- [x] `AC-01` React управляет оболочкой, деревом файлов, панелью инструментов,
  вкладками, конфликтами, диалогом создания, диагностикой, уведомлениями и
  адаптивным состоянием.
- [x] `AC-02` CodeMirror остаётся императивной поверхностью внутри компонента
  React и корректно освобождает ресурсы.
- [x] `AC-03` HTTP API, конфликты CAS, атомарное сохранение, предпросмотр и
  файловые ограничения сохраняют поведение и защитные проверки.
- [x] `AC-04` Клавиатура, фокус, ошибки, создание и мобильный режим покрыты
