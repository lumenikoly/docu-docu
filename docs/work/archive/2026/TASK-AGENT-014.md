...
```text
Начать работу          [встроенный агент] [копировать промт] [дополнить]
Спросить               [встроенный агент] [копировать промт]
Уточнить постановку    [встроенный агент] [копировать промт] [дополнить]
```

Пример для `in-progress` без активного сеанса Agent Session:

```text
Продолжить работу      [встроенный агент] [копировать промт] [дополнить]
Спросить               [встроенный агент] [копировать промт]
Что делать дальше      [встроенный агент] [копировать промт] [дополнить]
```
...
Спросить               [встроенный агент недоступен] [копировать промт]
```
...
Каждое действие отображается одной строкой. Справа находятся компактные
элементы управления: запуск во встроенном агенте, копирование промта для
внешнего агента и, для фиксированного промта, одноразовое дополнение.

Например:

```text
Взять в работу    [Agent Console] [копировать промт] [дополнить]
```
...
* [x] `AC-01` Каждая каноническая страница рабочей задачи в локальном `serve`
  показывает серверный набор доступных действий.
* [x] `AC-02` Клиентская часть не вычисляет допустимые действия из статуса
  автомата, готовности или зависимостей.
* [x] `AC-03` Рабочая область и страница задачи используют один HTTP-клиент и
  контроллер, не дублируя обработчики действий.
* [x] `AC-04` Интерфейс показывает серверный порядок действий плоским списком,
  а рядом с каждым действием доступны отдельные значки Agent Console, handoff
  и одноразового дополнения для фиксированного промта.
* [x] `AC-05` `Ask` использует доступное диалоговое окно с полем ввода вместо
...
* [x] `AC-06` Активный сеанс этой задачи показывает достоверное состояние и
  открывается через единый индикатор; запуск действия не раскрывает консоль
  автоматически и не создаёт повторный сеанс.
* [x] `AC-07` Сеанс другой задачи блокирует только доставку в Agent Console:
...
* [x] `AC-08` После изменения интерфейс перечитывает достоверный снимок задачи
  и действий, а устаревшая контрольная сумма корректно обрабатывается как
  конфликт.
* [x] `AC-09` Мягкая навигация закрывает открытое окно, отменяет ожидающий
...
* [x] `AC-10` Навигация с клавиатуры, управление фокусом, текстовые состояния и
  подписи значков соответствуют доступному шаблону взаимодействия.
* [x] `AC-11` Во время работы связанного агента нельзя повторно запустить
  действие Agent Console; handoff остаётся доступен, а продолжение появляется в
  `idle` и создаёт новый turn.
* [x] `AC-12` Ручная новая сессия не считается связанной с открытой задачей и
  очищает визуальное содержимое предыдущего запуска; запоздавшая проекция не
  возвращает устаревший индикатор.
* [x] `AC-13` Для действия с фиксированным промтом пользователь может добавить
  произвольный текст и выбрать доставку; сервер применяет дополнение только к
  этому запросу и не использует его при следующем запуске.
...
* `AC-13` → `go test ./internal/app -run 'TestTaskActionAddsOneTimeInstruction' && npm --prefix web test`
* `QUALITY` → `make check`
