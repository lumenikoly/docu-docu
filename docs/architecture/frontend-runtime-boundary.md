...
Go встраивает начальный список действий в Task Workspace и на страницу рабочей
задачи, а перед выполнением общий контроллер браузера запрашивает актуальную
проекцию через Task Actions API. Сервер задаёт порядок, главное действие и
доступность `agent-console` и `handoff`; браузер не строит инструкцию, не
выводит разрешение из статуса и не связывает идентификатор действия с
...
- Task Workspace не владеет смыслом действий, изменением задачи или правилами
  доставки; он показывает готовую серверную проекцию.
