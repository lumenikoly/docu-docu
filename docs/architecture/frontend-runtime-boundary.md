...
Go встраивает начальный список действий в Task Workspace, а перед выполнением
браузер запрашивает актуальную проекцию через Task Actions API. Браузер выбирает
`agent-console` либо `handoff`, но не строит инструкцию, не выводит разрешение
из статуса и не связывает идентификатор действия с конкретным поставщиком.
