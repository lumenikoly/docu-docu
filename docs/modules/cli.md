...
### BR-CLI-006: Выбранная локаль является рабочим контекстом

Любой каталог из `locales.*` поддерживает полный CLI и task workflow. Команда
строит одноязычную Model выбранного root и меняет только его файлы. Совпадающие
ID и статусы в других локалях не сравниваются и не синхронизируются.
...
активной локали. Видимые читателю значения из исходных документов сохраняются
