...
Здесь можно быстро узнать, что уже умеет Toudocu, и перейти к подробной
инструкции. Источником служит Markdown; JSON и собранный портал всегда можно
создать заново. Toudocu меняет исходные файлы только после явного действия в
`serve` или отдельной команды записи.

Если вы встраиваете Toudocu в другой инструмент, начните с [карты API](api.md).
...
всей ветки, но изменять файлы нельзя. В основном `serve`, открытом на локальном
адресе, действие «Начать работу» после повторной проверки переводит готовую
задачу в работу. Для
...
- `$toudocu translate <target-locale> --from <source-locale>` — переводит явно
  выбранную пару локалей одним выбранным способом;
...
Каждый каталог локали является независимым рабочим источником. Обычная операция
читает только активную локаль, а перевод — явно выбранную пару. Подробный путь описан в
