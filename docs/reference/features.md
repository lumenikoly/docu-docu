...
## Встроенная сессия агента

Основной `serve` на loopback-адресе может запустить одну структурированную
сессию установленного Codex для готовой задачи. Панель агента разделяет разговор
и поток выполняемых команд, восстанавливает доступную историю после перехода
между страницами и позволяет направить или остановить ответ. Выбор модели,
уровня рассуждения и полного доступа применяется только к следующей сессии;
расширение доступа требует отдельного подтверждения.

Toudocu не маскирует терминал под сессию агента. Терминал проекта запускает
обычную оболочку в корне репозитория, а команды и аргументы вводит пользователь.
Оба режима могут работать одновременно и имеют независимые жизненные циклы.
Если структурированная интеграция недоступна, интерфейс предлагает терминал или
готовый текст для передачи внешнему агенту.
