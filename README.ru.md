...
Markdown удобно хранить рядом с кодом, но со временем такой документации
становится трудно доверять. Ссылки ломаются, решения и задачи расходятся,
новые участники заново собирают картину проекта, а агенту приходится вручную
передавать требования и команды проверки.
...
|---|---|
| Создать документацию для существующего проекта | Отправьте агенту `$toudocu init` |
| Сверить всю документацию с проектом | Отправьте агенту `$toudocu refresh` |
| Обновить только документы, затронутые текущими изменениями | Отправьте агенту `$toudocu refresh diff` |
| Проверить существующий каталог Markdown | Выполните `toudocu check ./docs` |
| Читать, править и обсуждать документацию локально | Выполните `toudocu serve ./docs` |
| Передать готовую задачу установленному агенту | Откройте задачу в локальном портале и запустите Agent Console |
| Опубликовать документацию | Выполните `toudocu build ./docs --output ./site --clean` |

Записи с начальным `$toudocu` — это сообщения AI-агенту с установленным
навыком, а не команды терминала. Для обычной работы можно писать и своими
...
Toudocu поставляется со встроенным навыком для поддерживаемых агентов
разработки. Агент может изучить существующий репозиторий, создать минимальную
документацию и затем обновлять её вместе с проектом.
...
Навык не заставляет проект следовать заранее придуманной архитектуре: агент
должен опираться на то, что действительно существует в коде и репозитории.

Toudocu не привязан к конкретному агенту: CLI и процесс работы с Markdown можно
использовать с любым AI-агентом и средой его запуска.

Встроенный навык можно автоматически установить для **Codex**, **Claude Code**
и **GitHub Copilot**.
...
- Mermaid-диаграммы;
- стандарты и эксплуатационные инструкции;
- дорожная карта и рабочие задачи.

Проверку можно запускать локально или сделать обязательной частью CI.
...
- сообщения о проблемах после изменений;
...
Toudocu сохраняет такой запрос в локальной очереди. Установленный навык может
получить его через CLI, обработать в контексте проекта и вернуть ответ в
исходное обсуждение.
...
Каталог `site/` можно опубликовать, например, через GitHub Pages или другой
статический хостинг. На сервере после этого не нужен ни Toudocu, ни отдельная
серверная часть.
...
### Работа с агентом без ручного переноса контекста

В основном локальном портале готовую задачу можно передать установленному
**Codex** или **OpenCode**. Toudocu повторно проверяет состояние задачи и
передаёт агенту её актуальные требования, зависимости и ограничения.

Agent Console показывает разговор отдельно от выполняемых команд. Во время
работы можно дополнить поручение, остановить текущий ответ или завершить всю
сессию. Панель остаётся открытой при переходе между задачами, документацией,
изменениями и редактором.

Полный доступ включается только после отдельного подтверждения. Если встроенный
агент не подходит, то же поручение можно скопировать для внешнего агента или
выполнить через отдельный терминал проекта. Agent Console работает только в
основном `serve` на локальном адресе и недоступна в опубликованном портале.

### Равноправные рабочие каталоги локалей

Встроенный навык умеет поддерживать переводы документации:
...
Перейдите в корень проекта и установите встроенный навык.
...
По умолчанию навык устанавливается для текущего проекта. Если хотите
использовать его во всех проектах, добавьте `--scope user`.
...
Навык добавляет удобные сценарии работы, но не является обязательной
зависимостью Toudocu.
...
Агент анализирует изменения относительно `HEAD`, включая индекс Git, рабочий
каталог и новые неотслеживаемые файлы, а затем проверяет связанные документы.

Это удобно перед коммитом или запросом на слияние.
...
или для изменений исходника относительно выбранной ссылки Git:
...
В обычной работе эти низкоуровневые команды вызывает установленный навык,
поэтому пользователю не нужно вручную переносить содержание обсуждения между
порталом и агентом.
...
| Установить или обновить навык | `toudocu skill install\|update` |
...
Toudocu не требуется на рабочем сервере.
...
Toudocu намеренно не поддерживает часть расширений, которые усложняют
безопасную и предсказуемую обработку Markdown: сырой HTML, метаданные в начале
файла (front matter), атрибуты Markdown, сноски и активные ресурсы
SVG/XML/HTML.
...
`.toudocu/config.yml` задаёт каталоги локалей, внешний вид портала и поведение
раздела «Изменения».
