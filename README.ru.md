...
Если вы работаете с Toudocu через AI-агента, обычно достаточно нескольких команд:

| Когда использовать | Команда |
| --- | --- |
| **Подключаете Toudocu к проекту впервые** | `$toudocu init` |
| **Хотите проверить и обновить всю документацию** | `$toudocu refresh` |
| **Изменили код и хотите обновить только связанную документацию** | `$toudocu refresh diff` |
| **Нужно прояснить требования или решение до реализации** | `$toudocu clarify <тема>` |
| **Нужно обновить перевод документации** | `$toudocu translate <язык> --all-stale` |
| **Нужно перевести только текущие изменения** | `$toudocu translate diff` |

Для обычной работы специальную команду указывать не обязательно. Можно просто попросить агента, например: **«обнови документацию для этой функции»**, **«проверь документацию после этих изменений»** или **«подготовь контекст задачи TASK-AUTH-123»** — установленный Toudocu skill подключится, когда это необходимо.

Если Toudocu в проекте ещё не настроен, начните с `$toudocu init`.
...
### Равноправные рабочие каталоги локалей
...
Каждая настроенная локаль является независимым рабочим каталогом с записью. Toudocu не синхронизирует содержимое или состояния задач между локалями автоматически.

Исходной и целевой может быть любая пара разных настроенных локалей.
...
или для изменений исходника относительно Git ref:
...
Текущие проекты используют контракт документации v3.

`.toudocu/config.yml` задаёт равноправные locale roots, внешний вид портала и Changes.
...
locales:
  ru:
    root: docs
    sections: # полный набор из 13 разделов; см. справочник
